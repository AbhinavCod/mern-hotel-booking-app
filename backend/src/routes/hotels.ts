...
// Stripe Payment Intenet end-point
...
router.post("/:hotelId/bookings",verifyToken, async (req:Request,res:Response)=>{
  try {
    ...
    // console.log(paymentIntent.metadata.hotelId,req.params.hotelId);
    // console.log(paymentIntent.metadata.userId,req.userId);
    ...
    if(paymentIntent.status !== "succeeded"){
      return res.status(400).json({message:`Payment status not succeeded. Status : ${paymentIntent.status}`});
    }
